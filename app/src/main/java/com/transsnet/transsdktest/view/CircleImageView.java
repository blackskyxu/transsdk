...
public class CircleImageView extends AppCompatImageView {
    private final static String TAG = "CircleImageView";
    //缩放类型
...
//        super.onDraw(canvas);
    ...
    private void drawCircleImage(Canvas canvas) {
        ...
        float borderRadius = Math.min((mBorderRect.height() - mBorderWidth) / 2, (mBorderRect.width() - mBorderWidth) / 2);
        ...
        float drawableRadius = Math.min(mDrawableRect.height() / 2, mDrawableRect.width() / 2);
        //设置渲染器的变换矩阵也即是mBitmap用何种缩放形式填充
        updateShaderMatrix(mBitmap, mBitmapShader);
        //手动触发ondraw()函数 完成最终的绘制
//        invalidate();
        //绘制内圆形 图片 画笔为mBitmapPaint
        canvas.drawCircle(getWidth() / 2, getHeight() / 2, drawableRadius, mBitmapPaint);
        //如果圆形边缘的宽度不为0 我们还要绘制带边界的外圆形 边界画笔为mBorderPaint
        if (mBorderWidth != 0) {
            canvas.drawCircle(getWidth() / 2, getHeight() / 2, borderRadius, mBorderPaint);
        }

        if (!(getDrawable() instanceof BitmapDrawable)) {
            Log.i(TAG, " does not recycle Bitmap now");
            //if recycled, bitmap will report:
...
//            mBitmap.recycle();
     ...
     */
    private void updateShaderMatrix(Bitmap mBitmap, BitmapShader mBitmapShader) {
        float scale;
        float dx = 0;
        float dy = 0;
        int mBitmapWidth;   // 位图宽度
        int mBitmapHeight;  // 位图高度
